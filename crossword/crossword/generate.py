...
class CrosswordCreator():
    ...
    def enforce_node_consistency(self):
        ...
        """
        for var in self.crossword.variables:
            # Filter out words from the domain that don't match the variable's length
    ...
    def revise(self, x, y):
        ...
        if overlap:
            i, j = overlap
            # Iterate over words in domain of variable x
            for word_x in list(self.domains[x]):
                # Check if there exists a word in domain of variable y that agrees on the overlap
                if not any(word_x[i] == word_y[j] for word_y in self.domains[y]):
                    self.domains[x].remove(word_x)  # Remove inconsistent word from domain of x
                    revised = True
            return revised
            ...
            for neighbor in self.crossword.neighbors(var):
                if neighbor not in assignment:
                    for word in self.domains[neighbor]:
                        if word[var.overlaps[neighbor][1]] != value[var.overlaps[neighbor][0]]:
                            count += 1
            return count
