...
def transition_model(corpus, page, damping_factor):
    ...
    if not corpus[page]:
        # If the current page has no outgoing links, choose from all pages uniformly
        prob = 1 / num_pages
        for page in corpus:
            distribution[page] = prob
        return distribution
    ...
    while True:
        ...
        for page in corpus:
            ...
            for p, links in corpus.items():
                if page in links:
                    num_links = len(links)
                    new_page_rank += damping_factor * page_ranks[p] / num_links
